...
import com.sleepwalker.sleeplib.gg.essential.universal.UPacket
import com.sleepwalker.sleeplib.gg.essential.universal.utils.MCITextComponent
...
class UMessage {
    ...
    private fun parseMessage() {
        ...
        messageParts.forEach { _chatMessage.appendElem(it) }
    }
}

private val printChatMessageWithOptionalDeletion: MethodHandle? = try {
    val method = NewChatGui::class.java.declaredMethods.find { method ->
        ...
        "No method with parameters (${MCITextComponent::class.java.name}, int) in ${NewChatGui::class.java.name}."
    )
