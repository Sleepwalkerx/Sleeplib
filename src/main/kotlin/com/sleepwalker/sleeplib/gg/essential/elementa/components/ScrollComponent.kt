...
import com.sleepwalker.sleeplib.gg.essential.elementa.utils.bindLast
...
class ScrollComponent @JvmOverloads constructor(
    ...
    var horizontalDragBeginPos = -1f
    var verticalDragBeginPos = -1f

    /* DEV ====== */
    var verticalScrollBarMaxSize: SuperConstraint<Float> = 1.pixel
    var horizontalScrollBarMaxSize: SuperConstraint<Float> = 1.pixel
    /* DEV ====== */
    ...
    private fun updateScrollBar(scrollPercentage: Float, percentageOfParent: Float, isHorizontal: Boolean) {
        ...
        if (isHorizontal) {
            component.setWidth(  MaxConstraint(RelativeConstraint(clampedPercentage), horizontalScrollBarMaxSize))
        } else {
            component.setHeight(MaxConstraint(RelativeConstraint(clampedPercentage), verticalScrollBarMaxSize))
        }
