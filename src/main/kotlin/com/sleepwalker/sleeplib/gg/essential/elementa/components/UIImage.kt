...
import com.sleepwalker.sleeplib.gg.essential.elementa.components.image.*
import com.sleepwalker.sleeplib.gg.essential.elementa.components.UIImage.Companion
...
open class UIImage @JvmOverloads constructor(
    ...
    override fun drawImage(matrixStack: UMatrixStack, x: Double, y: Double, width: Double, height: Double, color: Color) {
        //DEV drawTexture(matrixStack, texture!!, color, x, y, width, height, textureMinFilter.glMode, textureMagFilter.glMode)
        when {
    ...
    companion object {
        ...
        /* DEV */
        @JvmStatic
        fun ofMCResource(path: ResourceLocation): UIImage {
            return UIImage(CompletableFuture.supplyAsync {
                ImageIO.read(Minecraft.getInstance().getTextureManager().resourceManager.getResource(path).inputStream)
            })
        }
        /* === */
